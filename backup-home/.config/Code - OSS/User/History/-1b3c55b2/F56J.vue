    ...
    <div @click.stop="close" class="container-payments" :class="{fullscreen:data.payments_fullscreen}">
        ...
        <template v-if="data.attendance">
            ...
            <div @click.stop class="bad-attendance">
                ...
                <select @change.stop="update_attendance_package($event.target.value,data.attendance,null,null)">
                    <template v-for="(payment, payment_index) in data.payments">
                        <template v-for="(package_0, package_index) in payment.package">
                            <option :value="package_0.id"> {{package_0.id}} | Payment: {{ Object.keys(data.payments).length - payment_index}} | Package: {{ Object.keys(payment.package).length - package_index }}</option>
                        </template> 
                    </template>
                </select>
