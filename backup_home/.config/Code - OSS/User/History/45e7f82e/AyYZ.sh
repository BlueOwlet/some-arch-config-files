...
backup_certificates(){
    ...
    sudo cp -r /etc/letsencrypt/* backup/certbot/
    sudo chown -R $USER:$USER backup/certbot
    sudo rm -r backup/certbot/accounts backup/certbot/live backup/certbot/renewal
}

restore_certificates(){
    sudo cp -r backup/certbot/* /etc/letsencrypt/
...
run_certbot(){
    ...
    case $option in
        ...
        3)
            ...
            ;;
        4)
            # sudo certbot --nginx --cert-name vyda.mx -d vyda.mx
            sudo certbot --nginx --cert-name vyda.mx -d *.vyda.mx
            ;;
        5)
            # sudo certbot certonly --manual --preferred-challenges=dns -v -d vyda.mx
            sudo certbot certonly --manual --preferred-challenges=dns -v -d *.vyda.mx
            # sudo certbot --nginx -d vyda.mx
            sudo certbot --nginx -v --cert-name vyda.mx
            ;;
