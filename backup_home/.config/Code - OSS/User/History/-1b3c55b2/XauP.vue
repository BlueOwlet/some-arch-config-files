    ...
    <div @click.stop="close" class="container-payments" :class="{fullscreen:data.payments_fullscreen}">
        ...
        <template v-if="data.attendance">
            ...
            <div @click.stop class="bad-attendance">
                ...
                <input @change.stop="update_attendance(data.attendance)" v-model="data.attendance.admin_check" type="checkbox">
                ...
                <select @change.stop="update_attendance_package($event,data.attendance,null,null)">
                    ...
                    <template v-for="(payment, payment_index) in data.payments">
                        <template v-for="(package_0, package_index) in payment.package">
                            <option :value="package_0.id"> Payment: {{ Object.keys(data.payments).length - payment_index}} | Package: {{ Object.keys(payment.package).length - package_index }}</option>
                        </template> 
                    </template>
                </select>                
                            ...
                            <table>
                                ...
                                <tbody>
                                    ...
                                    <tr class="attendance" :id="attendance.id" v-for="attendance in package_.attendance">
                                        <td>
                                            <input @change.stop="update_attendance(data.attendance)" v-model="attendance.admin_check" type="checkbox">
                                        </td>
                                        <td>
                                            {{ attendance.id }}
                                        </td>
...
<script setup>
    ...
    async function update_attendance_package(e,attendance,package_,payment){
        ...
        data.payments.forEach(new_payment => {
            if(new_payment.id == payment.id){
                new_payment.show=true
            }
        })
    ...
    async function show_payment(payment){
        ...
        let response=null
        if (payment.show){
            response = await axios.post('/manager/get_payment/',{payment_id:payment.id})
            Object.assign(payment,response.data)
        }else{
            payment=payment.id
        }
