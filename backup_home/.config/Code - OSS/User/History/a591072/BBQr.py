...
def json_payments(student):
    ...
    serializer=get_serializer('Admin-Maker-Payment-Plain')
...
class Manager(viewsets.GenericViewSet):
    queryset=None
...
class DashboardViewset(viewsets.GenericViewSet):
    ...
    def filtered_bad_attendances(self,request,*args,**kwargs):
        ...
        month_attendances=Attendance.objects.filter(Q(datetime__gte=initial_datetime),Q(datetime__lte=final_datetime),Q(admin_check=False)).order_by('group','datetime')
        bad_attendances=[]
    ...
    def bad_attendances(self,request,*args,**kwargs):
        ...
        month_attendances=Attendance.objects.filter(Q(datetime__month=month),Q(datetime__year=year),Q(admin_check=False)).order_by('group','datetime')
...
class InstructorsViewset(
    ...
    def class_status(self,request,*args,**kwargs):
        ...
        if status in ['cancelled-on-time','reschedule']:
            students=group.student.all()
            for student in students:
                student_status(student,group,status)
...
class StudentsViewset(
    ...
    def class_status(self,request,*args,**kwargs):
        ...
        attendance=student_status(student,group,status)
...
def student_status(student,group,status):
